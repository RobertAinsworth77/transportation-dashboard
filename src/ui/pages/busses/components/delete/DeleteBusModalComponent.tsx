import './DeleteBusModalStyles.scss';
...
import DeleteBusModalComponentProps from './DeleteBusModalComponentProps';

const DeleteBusModalComponent: FC<DeleteBusModalComponentProps> = ({ done }) => {
    ...
    <form onSubmit={handleSubmit(_handleDelete)}>
      <div className="row">
        <div className={`col-12 form-group ${errors.confirm ? 'error' : ''}`}>
          <label>{i18n(KeyWordLocalization.DeleteBusDescription)}</label>
          ...
          }))} className="form-control my-2" placeholder={i18n(KeyWordLocalization.DeleteBusPlaceholder)} />
...
export default DeleteBusModalComponent;
